...
export class GuildStageInstanceManager extends CachedManager<Snowflake, StageInstance> {
    ...
    public async fetch(id: Snowflake, options?: FetchOptions) {
        ...
        return this.cache._add(stageInstance.id, stageInstance);
    }

    public async create(data: CreateStageInstanceData) {
        const stageInstance = await this.client.caches.stageInstances.create(data);
