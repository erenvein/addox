...
export async function GuildScheduledEventDataResolver(
    scheduledEvent: CreateGuildScheduledEventData | EditGuildScheduledEventData
) {
    ...
    if ('image' in res) {
        //@ts-ignore
        res.image = await DataResolver.resolveImage(res.image, 'image/jpeg');
    }
